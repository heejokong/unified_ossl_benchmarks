...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data',
    ...
    """
    from semilearn.datasets import get_cifar_openset, get_imagenet30, get_semi_inat
    ...
    elif dataset == 'semi_inat':
        lb_dset, ulb_dset, eval_dset, eval_full_dset = get_semi_inat(args, algorithm, dataset, num_labels,
                                                                      num_classes, data_dir=data_dir)
        if eval_open:
            test_dset = {'full': eval_full_dset}
    ...
    ####################################################################################################
    from semilearn.datasets import get_ood
    use_ood_data = False
    if eval_open and use_ood_data:
        if dataset == "cifar10_openset":
            id = dataset.split('_')[0]
            ood_names = ["svhn", 'cifar100', 'lsun', 'imagenet']
        elif dataset == "cifar100_openset":
            id = dataset.split('_')[0]
            ood_names = ["svhn", 'cifar10', 'lsun', 'imagenet']
        elif dataset == 'imagenet30':
            id = dataset
            ood_names = ['lsun', 'dtd', 'cub', 'flowers102', 'caltech_256', 'stanford_dogs']
        for ood_name in ood_names:
            dset = get_ood[ood_name](args, id, data_dir=data_dir)
            test_dset[ood_name] = dset
    ####################################################################################################
