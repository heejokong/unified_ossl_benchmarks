...
def split_ossl_data(args, data, target, num_labels, num_all_classes, seen_classes, index=None, include_lb_to_ulb=True):
    ...
    args.novel_classes = args.num_all_classes - args.num_classes
    # seen_classes should be a set
    data, target = np.array(data), np.array(target)
    target = reassign_target(target, num_all_classes, seen_classes)

    if args.mm_ablation:
        lb_idx, ulb_idx = sample_lb_ulb_data_w_ratio(args, data, target, num_labels, len(seen_classes), index)
        if include_lb_to_ulb:
            ulb_idx = np.array(sorted(list(set(ulb_idx).union(set(lb_idx)))))
            return data[lb_idx], target[lb_idx], data[ulb_idx], target[ulb_idx]
        else:
            return data[lb_idx], target[lb_idx], data[ulb_idx], target[ulb_idx]
...
def sample_lb_ulb_data_w_ratio(args, data, target, num_labels, num_classes, index=None, load_exist=False):
    ''' '''
    dump_dir = os.path.join(base_dir, 'data', args.dataset, 'labeled_idx')
    os.makedirs(dump_dir, exist_ok=True)
    lb_dump_path = os.path.join(dump_dir, f'lb_labels{args.num_labels}_seed{args.seed}_idx.npy')
    ulb_dump_path = os.path.join(dump_dir, f'ulb_labels{args.num_labels}_seed{args.seed}_ratio{args.ratio}_idx.npy')
    if os.path.exists(lb_dump_path) and os.path.exists(ulb_dump_path) and load_exist:
        lb_idx = np.load(lb_dump_path)
        ulb_idx = np.load(ulb_dump_path)
        return lb_idx, ulb_idx
    # 
    lb_samples_per_class = int(num_labels / args.num_classes)
    ulb_samples_per_class_in = int(args.num_unlabeled * (1.0-args.ratio)) // args.num_classes
    ulb_samples_per_class_out = (args.num_unlabeled - (ulb_samples_per_class_in * args.num_classes)) // args.novel_classes
    ...
    for c in range(num_classes):
        idx = np.where(target == c)[0]
        idx = np.random.choice(idx, lb_samples_per_class + ulb_samples_per_class_in, False)
        lb_idx.extend(idx[:lb_samples_per_class])
        ulb_idx.extend(idx[lb_samples_per_class:lb_samples_per_class+ulb_samples_per_class_in])
    # 
    for c in range(num_classes, args.num_all_classes):
        idx = np.where(target == c)[0]
        idx = np.random.choice(idx, ulb_samples_per_class_out, False)
        ulb_idx.extend(idx)

    np.save(lb_dump_path, np.array(lb_idx))
    np.save(ulb_dump_path, np.array(ulb_idx))
    return np.array(lb_idx), np.array(ulb_idx)


def sample_labeled_data(args, data, target, num_labels, num_classes, index=None, load_exist=False):
    ...
    if os.path.exists(dump_path) and load_exist:
